...
public class AccreditationProcessMigrationService {
    ...
    public void initialize(ExportDegreeProcessBean form) {
        ...
        if (Strings.isNullOrEmpty(form.getFormId())) {
            ...
            try {
                processes = invoke(webResource().path(API_PROCESS));
            ...
            if (!Strings.isNullOrEmpty(form.getDegreeFile().getDegreeCode())
                ...
                for (Object object : forms) {
                    ...
                    if ("Guião para a auto-avaliação".equals(jsonForm.get("name"))) {
    ...
    public List<String> uploadCompetenceCourses(DegreeFile degreeFile) {
        ...
        for (Object object : invoke(webResource().path(API_FOLDER).queryParam("formId", formId))) {
            ...
            if ("6.2.1. Ficha das unidades curriculares".equals(folder.get("name"))) {
                ...
                for (Entry<JSONObject, String> json : buildCompetenceCoursesJson(degreeFile).entrySet()) {
                    ...
                    if (status == 201) {
                        output.add(status + ": " + json.getKey().get("q-6.2.1.1") + ": " + json.getValue());
                    } else {
    ...
    protected Map<JSONObject, String> buildCompetenceCoursesJson(DegreeFile degreeFile) {
        ...
        degreeFile.getCurricularUnitFileSet().forEach(curricularUnitFile -> {
            ...
            json.put("q-6.2.1.1", curricularUnitFile.getFileName());
            json.put("q-6.2.1.2", cut(message("label.responsibleTeacherAndTeachingHours"),
                    curricularUnitFile.getResponsibleTeacherAndTeachingHours(), output, 100));
            json.put("q-6.2.1.3", curricularUnitFile.getOtherTeachersAndTeachingHours());
    ...
    public List<String> uploadTeacherCurriculum(DegreeFile degreeFile) {
        List<String> output = new ArrayList<String>();
        for (Object object : invoke(webResource().path(API_FOLDER).queryParam("formId", formId))) {
            JSONObject folder = (JSONObject) object;
        ...
        degreeFile.getTeacherFileSet().forEach(teacherFile -> {
            ...
            JSONObject file = new JSONObject();
            {
                ...
                if (teacherFile.getDegreeInstitution() != null) {
                    file.put("instituicao_conferente", cut("instituicao_conferente", teacherFile.getDegreeInstitution(), output, 200));
                } else {
                ...
                file.put("form-professional",
                        getJsonActivities(teacherFile.getOtherProfessionalActivitySet(), "profession", "profession", 200, output));
                ...
                teacherFile.getA3esTeachingServiceSet().forEach(teachingService -> {
                    ...
                    lecture.put("curricularUnit",
                            cut("curricularUnit", teachingService.getCurricularUnitName(), output, 100));
